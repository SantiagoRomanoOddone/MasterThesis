...
from models.lightgbm.lightgbm_model import LightGBMRegressor
...
def lightgbm(cluster_data):
    print('[START] LightGBM model')
    ...
    final_results.rename(columns={'cant_vta_pred': 'cant_vta_pred_lgbm_pdv_sku'}, inplace=True)
    print('[END] LightGBM model')
    return final_results


def lightgbm_by_product(cluster_data):
    print('[START] LightGBM model by product')
    ...
    final_results = pd.concat(results, ignore_index=True)
    final_results.rename(columns={'cant_vta_pred': 'cant_vta_pred_lgbm_sku'}, inplace=True)
    print('[END] LightGBM model by product')
    return final_results
...
if __name__ == '__main__':
    ...
    results = lightgbm(features)
    results_sku = lightgbm_by_product(features)
