...
def train_dlinear_model(df_train, ts_code, start_date, freq, prediction_length):
    ...
    estimator = DLinearEstimator(
        ...
        hidden_dimension=None,  # Optional: Dimension of the hidden layer (default is None, which uses input dimension)
        ...
        kernel_size=25,  # Optional: Size of the convolutional kernel (default is 25)
...
# Process results
def process_dlinear_results(tss, forecasts, df_input, start_test, freq, prediction_length, sku):
    all_results = []
    ...
    for i, (tss_series, forecast) in enumerate(zip(tss, forecasts)):
        ...
        results = pd.DataFrame({
            ...
            'cant_vta': latest_tss,
            'cant_vta_pred_dlinear': forecast.mean,
...
def dlinear_main(features):
    ...
    for sku in valid_skus:
      ...
      # Process results
      final_results = process_dlinear_results(
          tss=tss,
          forecasts=forecasts,
          df_input=df_input,
...
if __name__ == "__main__":
    ...
    final_results = dlinear_main(filtered)
    print(final_results)