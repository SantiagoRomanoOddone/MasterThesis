...
def train_patchtst_model(df_train, ts_code, start_date, freq, prediction_length):

    # Define the estimator
    estimator = PatchTSTEstimator(
        prediction_length=prediction_length,  # Required: Length of the prediction horizon
        patch_len=16,  # Required: Length of each patch
        context_length=None,  # Optional: Length of the context window (defaults to prediction_length if None)
        stride=8,  # Optional: Stride for patch creation (default is 8)
        padding_patch="end",  # Optional: Padding strategy for patches ("end" or "begin")
        ...
        num_feat_dynamic_real=0,  # Optional: Number of dynamic real-valued features (default is 0)
...
# Process results
def process_patchtst_results(tss, forecasts, df_input, start_test, freq, prediction_length, sku):
    all_results = []
    ...
    for i, (tss_series, forecast) in enumerate(zip(tss, forecasts)):
        ...
        results = pd.DataFrame({
            ...
            'cant_vta': latest_tss,
            'cant_vta_pred_patchtst': forecast.mean,
...
def patchtst_main(features):
    ...
    for sku in valid_skus:
      ...
      # Train the model
      try:
          predictor = train_patchtst_model(
              df_train=df_train,
              ts_code=ts_code,
      ...
      # Process results
      final_results = process_patchtst_results(
          tss=tss,
          forecasts=forecasts,
          df_input=df_input,
...
if __name__ == "__main__":
    ...
    final_results = patchtst_main(filtered)
    print(final_results)